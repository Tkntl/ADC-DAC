 ...
 entity adc128s022 is
    ...
    port(
        ...
        adc_clk     : out std_logic;    --specified performance 0.8 Mhz - 3.2 Mhz
...
architecture rtl of adc128s022 is
    ...
    signal adc_clk_Quartus : std_logic;
begin

gen_adc_clk: process(rst_n,Mclk)
...
begin
    if rst_n = '0' then
        ...
        adc_clk <= clkDivCntr(4);
        adc_clk_Quartus <= clkDivCntr(4);
    end if;
end process gen_adc_clk;

fsm_adc: process(rst_n,adc_clk_Quartus)
...
begin
    ...
    if rst_n = '0' then
        ...
        adc_read <= '0';
        adc_stop <= '0';
    elsif rising_edge(adc_clk_Quartus) then
        case fsm is
            when IDLE =>
...
set_adc: process(rst_n,adc_clk_Quartus)
...
begin
    ...
    elsif falling_edge(adc_clk_Quartus) AND adc_set = '1' then
        adc_n_cs <= '0';
    ...
    elsif falling_edge(adc_clk_Quartus) AND adc_stop = '1' then
        adc_n_cs <= '1';
    end if;
end process set_adc;

read_data_adc: process(rst_n, adc_clk_Quartus)
...
begin
    if rst_n = '0' then
        ...
        Q <= (others => '0');
        Q_valid <= '0';
    elsif rising_edge(adc_clk_Quartus) AND adc_read = '1' then
        Q(cntr) <= adc_Dout;
        if cntr > 0 then
            Q_valid <= '0';
            cntr := cntr - 1;
